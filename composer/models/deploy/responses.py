...
class MarketHoursItem(BaseModel):
    ...
    market_close: Optional[str] = Field(None, description="Market close time (ISO 8601)")
...
class DeploySymphony(BaseModel):
    ...
    community_review_status: Optional[str] = Field(None, description="Community review status")
    ...
    rebalance_corridor_width: Optional[float] = Field(None, description="Rebalance corridor width")
...
class Deploy(BaseModel):
    ...
    symphony_id: Optional[str] = Field(None, description="Symphony ID")
    deploy_created_by: Optional[str] = Field(None, description="User who created the deploy")
    ...
    cash_change: Optional[float] = Field(None, description="Cash change")
    requested_cash_change: Optional[float] = Field(None, description="Requested cash change")
...
class DeployActionResponse(BaseModel):
    ...
    deploy_id: str = Field(description="Unique identifier for the deployment")
    deploy_time: Optional[str] = Field(None, description="When the deploy is scheduled")
    deploy_on_market_open: Optional[bool] = Field(
